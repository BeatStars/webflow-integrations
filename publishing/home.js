...
import { testimonials } from './json-data.js'
...
const fullPage = createApp({
    ...
    methods: {
        ...
        async handleSpecialKeys(element) {
            if (this.songTitle === this.songTitleBkp) return
    ...
    mounted() {
        this.trackGaEvent('pub_page_landed')
                ...
                for (let item of tracks) {
                    ...
                    this.selectedTracks.forEach(element => {
                        if (element.id === spotifyId) {
                            ...
                            if (!x) {
                                let input = item.querySelector('.integration_track-input')
                                input.value = element.percentageOwned
